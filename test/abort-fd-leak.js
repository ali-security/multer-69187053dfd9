...
// Count open file descriptors that point at a file inside `dir`. Linux exposes
// these via /proc/self/fd as symlinks to the file they hold open; a descriptor
// whose file has already been unlinked keeps the target but gains a
// " (deleted)" suffix. Once every upload has aborted and been cleaned up there
// must be no descriptor left in either form -- an open one on a still-present
// partial file is an orphan plus a leak, an open one on a deleted file is a
// pure leak. Returns -1 when /proc/self/fd is unavailable (non-Linux).
function countLeakedFds (dir) {
  ...
  try {
    ...
    prefix = fs.realpathSync(dir) + path.sep
  } catch (e) {
    return -1
  }
    ...
    function loop () {
      if (i++ >= attempts) {
        ...
        setTimeout(function () {
          ...
          assert.strictEqual(leaked, 0, 'leaked ' + leaked + ' upload-file descriptor(s) after ' + attempts + ' aborted uploads')
          done()
        }, 500)
        return
      }
