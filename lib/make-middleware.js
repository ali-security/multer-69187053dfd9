...
// Default cap on the numeric array index a field name may carry (a[10000]).
// Upstream leaves limits.fieldArrayIndexLimit unset (effectively Infinity), so
// a single field name can still materialize a huge sparse array unless the
// consumer opts in. Sealed builds apply this default so protection is on out of
// the box; it is far above any realistic form array size and callers can
// override or disable it via limits.fieldArrayIndexLimit (set Infinity to
// restore upstream behavior).
var DEFAULT_FIELD_ARRAY_INDEX_LIMIT = 10000
...
function makeMiddleware (setup) {
  return function multerMiddleware (req, res, next) {
    ...
    busboy.on('field', function (fieldname, value, { nameTruncated, valueTruncated }) {
      ...
      var fieldArrayIndexLimit = limits && Object.prototype.hasOwnProperty.call(limits, 'fieldArrayIndexLimit')
        ? limits.fieldArrayIndexLimit
        : DEFAULT_FIELD_ARRAY_INDEX_LIMIT
      if (exceedsArrayIndexLimit(fieldname, fieldArrayIndexLimit)) {
        return abortWithCode('LIMIT_FIELD_ARRAY_INDEX', fieldname)
      }

      // append-field can throw on a crafted field name -- most notably a
      // RangeError ("Invalid array length") when a name pushes onto an array
      // that a previous name already grew to the maximum length. The throw
      // happens inside a busboy event handler, so it escapes as an uncaught
      // exception and takes the process down; surface it as a request error
      // instead (GHSA-wc9g-mqfw-jrwm).
      try {
