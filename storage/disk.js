...
DiskStorage.prototype._handleFile = function _handleFile (req, file, cb) {
  ...
  that.getDestination(req, file, function (err, destination) {
    ...
    that.getFilename(req, file, function (err, filename) {
      ...
      // The descriptor is released before either of these fires: 'close' comes
      // after fs.close(), and a write stream only emits 'error' once its
      // destroy hook has closed the descriptor (or never opened one), so once
      // one of them fires _removeFile can unlink straight away.
      function forgetStream () { openStreams.delete(file) }
      outStream.once('close', forgetStream)
      outStream.once('error', forgetStream)
